...
DENSE_PPO   = {0: 75, 1: 95, 2: 95, 3: 90, 4: 100, 5: 85, 6: 90, 7: 95, 8: 85, 9: 75}
...
class BCVanillaEnv(gym.Env):
    ...
    def step(self, residual_action):
        ...
        return self._get_latent(), sparse_reward, done, info
...
print("\n" + "="*60)
print("BC + Vanilla PPO (Sparse Reward) Evaluation")
print("="*60)
...
for task_id in range(10):
    ...
    env   = BCVanillaEnv(task_id, bc_policy, task_emb)
    model = PPO.load(f'/234/outputs/ppo_bc_vanilla/task{task_id}/best_model/best_model.zip')
    ...
    dense = DENSE_PPO[task_id]
    print(f"  BC: {bc}%  |  BC+Vanilla PPO: {rate:.1f}%  |  BC+Dense PPO: {dense}%")
    env.close()
...
for task_id in range(10):
    ...
    van   = results[task_id]
    dense = DENSE_PPO[task_id]
    print(f"  Task {task_id}: BC={bc}%  Vanilla={van:.1f}%  Dense={dense}%")

print(f"\n  BC Average:           {np.mean(list(BC_BASELINE.values())):.1f}%")
print(f"  BC+Vanilla PPO Avg:   {np.mean(list(results.values())):.1f}%")
print(f"  BC+Dense PPO Avg:     {np.mean(list(DENSE_PPO.values())):.1f}%")
