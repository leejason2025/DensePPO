"""Single task PPO — takes --task argument for parallel launching."""
...
OUT_DIR   = f"/234/outputs/ppo/task{TASK_ID}"
...
# ── Train ─────────────────────────────────────────────────────────────────────
print(f"Task {TASK_ID}: loading BC policy...")
bc_policy, task_emb, cfg = load_bc_policy(TASK_ID)
waypoints = load_waypoints(TASK_ID)
print(f"Task {TASK_ID}: waypoints shape {waypoints.shape}")
